...
from Filler_filter.Filler_filter import (Filler_filter,
                                         create_language_detection_model)
# Load filters folders
from PII_filter.PII_filter import PII_filter, regex_dict
...
if st.button(label="✨ Validate!"):
    ...
    result_dict = Filler_filter(language_detection_model, input_dict['review'])
    if not result_dict['suspicious']:
        result_dict = PII_filter(input_dict['review'], regex_dict)

    if result_dict['suspicious']:
        st.error(
            f'💢 THE REVIEW IS SUSPICIOUS! \n  We think this review includes '
            f'{result_dict["filter_failed"]} ( Candidates: {result_dict["motive"]})')
